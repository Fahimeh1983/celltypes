...
def Get_Median_per_Type(df, ref_cl, groupby_col, merge_col, normalize = True):
    ...
    for g, data in groups:
        ...
        output_dict[g] = data.median(axis=0)  # Find median gene expression for each type
        if normalize:
...
def Log_CPM(df):
    ...
    '''
    Take a CPM dataframe and returns its log(CPM+1)

    parameters
    ----------
    df: a data frame of CMP values
