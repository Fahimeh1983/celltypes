...
class EmitterReceiverCoupled(nn.Module):
    ...
    def decoder2(self, first_second_embeddings, arm):
        ...
        second_embedding = torch.unbind(first_second_embeddings, dim=1)[1]
        out = self.decode_l2[arm](second_embedding)
        out = self.sigmoid(out)
        return out
    ...
    def forward(self, first_node, second_node, index_2_word_tensor):
        ...
        output1 = [None] * self.n_arms #place holder for the output of decoder
        output2 = [None] * self.n_arms #place holder for the output of decoder
...
def loss_AE1(output1, n_arms, n_nodes, first_node):
    '''
...
def loss_AE2(output2, n_arms, n_nodes, second_node):
    '''
    Take the output which is obtained from the decoder, this is basically coordinate of j and we want to predict j again
    like and autoencoder using BCE loss
    ...
    for arm in range(n_arms):
        # here we convert the index of j to its one-hot representation
        target = (second_node[arm] == torch.arange(n_nodes).reshape(1, n_nodes).to(device)).float()
        loss = nn.BCELoss()
...
def total_loss(first_second_node_embeddings, n_arms, output1, output2, n_nodes, first_node, second_node, lamda):
    '''
    ...
    Returns:
    '''
    AE_loss1 = loss_AE1(output1, n_arms, n_nodes, first_node)
    AE_loss2 = loss_AE2(output2, n_arms, n_nodes, second_node)
    ...
    return mean_dist, distance_loss, bothmvl, mvl, AE_loss1 ,AE_loss2,  lamda * (distance_loss / mvl) + (1 - lamda) * (AE_loss1 + AE_loss2)
...
path = "/Users/fahimehb/Documents/NPP_GNN_project/models/VISp/combined_layers/Interaction_mats_8.0/run2/"
walks = read_list_of_lists_from_csv(path + "/walk_20_50_again_binary.csv")
        ...
        for l in [0.99]: # lambda in the loss function
            ...
            for epoch in range(n_epochs):
                ...
                for batch_idx, all_data in enumerate(data_loader):
                    ...
                    optimizer.zero_grad()
                    all_node_emb, first_second_node_embeddings, output1, output2 = model(first_node, second_node, torch.tensor(
                        [i for i in index_2_word.keys()]).to(device))
                    d, dloss, bothmv, minv, AE1, AE2, loss = total_loss(
                        first_second_node_embeddings, n_arms, output1, output2, n_nodes, first_node, second_node, lamda)
                    loss.backward()
                    optimizer.step()
                    losses.append(loss.item())

                training_loss.append(np.mean(losses))
                AEloss1.append(AE1.item())
                AEloss2.append(AE2.item())
                ...
                if ((epoch % 100 == 0)):
                    ...
                    output_filename = prefix + "_AE_loss1.csv"
                    utils.write_list_to_csv(path + '/' + output_filename, AEloss1)

                    output_filename = prefix + "_AE_loss2.csv"
                    utils.write_list_to_csv(path + '/' + output_filename, AEloss2)
